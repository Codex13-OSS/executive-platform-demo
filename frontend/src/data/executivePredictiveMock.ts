...
export function buildExecutivePredictions(events: ExecutiveAgendaEvent[]): ExecutivePrediction[] {
  ...
  sortedEvents.forEach((event) => {
    ...
    if (event.status === 'critico') {
      predictions.push({
        ...
        recommendation: 'Preparar resumen, responsables y criterio de cierre antes del bloque.',
        confirmationLabel: 'Preparar resumen',
        eventId: event.id,
        eventTime: event.startTime,
      });
    }
  ...
  if (firstFreeWindow) {
    predictions.push({
      ...
      detail: 'LÍA detecta espacio disponible para seguimiento, documentación o resumen.',
