...
export default function DashboardView({ session, cards, onLogout, theme, onToggleTheme }: DashboardViewProps) {
  ...
  const nav = (
    <nav className="shell-nav-list">
      {views.map((v)=><button key={v} className={tab===v?'active':''} onClick={()=>{setTab(v);setDrawerOpen(false);}}>{v}</button>)}
    </nav>
  );
  ...
  return (
    <main className="dashboard-shell">
      ...
      <div className="dashboard-main">
        <button className="command-nav-btn" onClick={()=>setDrawerOpen(true)}>☰ Navegación</button>
        ...
        {tab==='agenda' && <section className="agenda-premium"><GlassPanel variant="strong" className="agenda-header"><p className="eyebrow">Agenda ejecutiva contextual</p><h2>Calendario inteligente semanal</h2></GlassPanel><div className="agenda-week">{days.map((d)=><button key={d.key} onClick={()=>setActiveDay(d.key)} className={activeDay===d.key?'active':''}><strong>{d.label}</strong><span>{d.date}</span><em>{d.meetings} reuniones</em><b className={d.status}>{d.status}</b></button>)}</div><div className="agenda-day-grid"><GlassPanel variant="default"><h3>Agenda del día</h3>{day.events.map((e)=><article key={e.time} className={`event-block ${e.priority.toLowerCase()}`}><div><b>{e.time}</b><span>{e.title}</span></div><small>{e.priority} · briefing {e.briefing}</small><div className="event-actions"><button onClick={()=>setBriefing(e.time)}>Generar briefing</button><button onClick={()=>addAlert('Recordatorio creado', `${e.title} (${e.time})`, 'Media','Agenda')}>Crear recordatorio</button><button>Ver contexto</button></div></article>)}</GlassPanel><GlassPanel variant="default"><h3>Espacios disponibles</h3>{day.slots.map((slot)=><div key={slot} className="free-slot"><span>{slot}</span><button onClick={()=>blockSlot(slot)}>Bloquear espacio</button></div>)}</GlassPanel></div></section>}
