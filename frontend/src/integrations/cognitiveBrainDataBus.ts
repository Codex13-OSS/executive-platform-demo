...
export function validateExecutiveCognitiveBrainDataBusSnapshot(
  ...
  const checks = [
    ...
    {
      id: 'minimum-domains-represented',
      passed: requiredDomains.every((domain) => snapshot.sourceDomains.includes(domain)),
      detail: 'All read-only domains are represented in the bus.',
    },
    {
      id: 'signals-valid-domain',
      passed: snapshot.brainSignals.every((signal) => allowedDomains.has(signal.domain)),
      detail: 'Each signal comes from an allowed domain.',
    },
    {
      id: 'core-flows-present',
      passed: snapshot.brainSignals.some((signal) => signal.domain === 'agenda')
        && snapshot.brainSignals.some((signal) => signal.domain === 'risks')
        && snapshot.brainSignals.some((signal) => signal.domain === 'documents'),
      detail: 'Agenda, risks and documents are represented in active brain signals.',
    },
    {
      id: 'no-endpoints-or-credentials',
      passed: !('endpoint' in snapshot) && !('credentials' in snapshot),
      detail: 'Snapshot shape includes no endpoint or credential fields.',
    },
  ];
  ...
  return {
    status: safeToRender ? 'safe' : 'warning',
    safeToRender,
